...
const interactionApiResponseMessages = [
  {
    type: 'progress',
    progress: {
      // type: 'status',
      content:
        'Hello, im an agent and I am going to choose a tool for your request!',
    },
    response: null,
  },
  {
  ...
  },
  {
    ...
    progress: {
      // type: 'status',
      content: 'adding these two numbers is pretty tough',
    },
    response: null,
  },
  {
    ...
    progress: {
      // type: 'status',
      content: 'tool call <adder> completed',
    },
    response: null,
  },
  {
...
//     catFacts[getRandomIndex(0, catFacts.length - 1)]
//   }\n\n\n`
// }
...
//     getRandomIndex(0, 1) ? 'Heads' : 'Tails'
//   }\n\n\n`
// }
...
function createInteractipApiEvents(request, response) {
  response.writeHead(200, {
    Connection: 'keep-alive',
    'Content-Type': 'text/event-stream',
    'Cache-Control': 'no-cache',
  ...
  const interval = setInterval(() => {
    if (eventsSent >= interactionApiResponseMessages.length) {
      clearInterval(interval)
      console.log('Sent all events, stopping.')
      response.write('id: -1\ndata:\n\n\n')
      response.end()
      return
    }
    ...
    const message = interactionApiResponseMessages[eventsSent]
...
http
  .createServer((request, response) => {
    ...
    switch (request.url) {
      ...
      //   break
      case '/interactionApi':
        createInteractipApiEvents(request, response)
        break
      default:
        // Unknown URL
